 ...
 *
 * --- MODIFIED FOR ELECTRON INTEGRATION ---
 */
...
(function() {
	...
	// --- NEW: Function to send the current state to the parent window ---
	function sendStateToParent() {
		if (window.parent) {
			// Create a clean, savable version of the contestants array
			var savableState = contestants.map(function(con) {
				return {
					image: con.image,
					score: con.score,
					oldScore: con.oldScore
				};
			});
			
			window.parent.postMessage({
				type: 'scoreboard:update',
				payload: savableState
			}, '*');
		}
	}
	...
	function createContestantEl(con, id) {
		...
		// This works perfectly with both file paths and Base64 strings
		...
		// --- MODIFIED: Image loading now uses FileReader to get Base64 ---
		frame.addEventListener("click", function() {
			var cb = function(evt) {
				if (fileInput.files && fileInput.files[0]) {
					var reader = new FileReader();
					reader.onload = function (e) {
						con.image = e.target.result; // This is the Base64 string
						fill.style.backgroundImage = "url(" + con.image + ")";
						sendStateToParent(); // Send update
					};
	...
	function transformContestants() {
		...
		var maxScore = contestants.length > 0 ? contestants[contestants.length - 1].score : 0;
		var maxCount = 1;
	...
	function play() {
		...
		if (!locked) {
			...
			resize();
			refreshContestants(); // Refresh to remove "X" buttons
		}
		...
		setTimeout(function() {
			...
			var loop = function(dt) {
				...
				if (dt - start < 2000) {
					...
					sendStateToParent(); // Save the new "oldScore"
				}
			};
	...
	// --- NEW: Listen for 'load' message from parent ---
	window.addEventListener('message', function(event) {
		if (event.data && event.data.type === 'scoreboard:load') {
			var loadedContestants = event.data.payload || [];
			if (loadedContestants.length > 0) {
				contestants = loadedContestants;
			} else {
				// Load default 5 if project is empty
				contestants = [];
				for (var i = 0; i < 5; ++i)
					addContestant();
			}
			refreshContestants();
			resize();
			sendStateToParent(); // Send the initial state back up
		}
	});

	// Default setup if no 'load' message is received (e.g., running standalone)
